...
func Deserialize(blob []byte) (*Library, error) {
...
func (c *Code) Find(subject string) (*Match, bool) {
	...
	return &Match{
		...
		Start: int(m.start),
		End:   int(m.end),
		Text:  subject[int(m.start):int(m.end)],
	}, true
}

// FindAll matches the subject against every pattern in the library using a
// single cgo call. The subject is normalized once and the loop runs inside C,
// avoiding thousands of cgo round trips.
func (l *Library) FindAll(subject string) []Match {

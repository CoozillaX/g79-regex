...
int bridge_find_all(
    ...
    size_t capacity)
{
    if (!lib || !subject || !results)
        return -1;

    /* Normalize the subject once; shared by every pattern. */
    NormalizeResult norm =
        normalize_subject(subject);
    ...
    /*
     * Only the whole match (ovector[0]/[1]) is needed, so a
     * single ovector pair is enough and is reused for the
     * entire call.
     */
    pcre2_match_data *match =
    ...
    for (size_t i = 0;
        ...
        /* pcre2_match uses the JIT path automatically when compiled. */
        int n =
            pcre2_match(
                rc->code,
                (PCRE2_SPTR)norm.text,
